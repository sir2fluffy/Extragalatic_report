...
pl.plot([-.1,.4,.4],[-22.6,-22.6,-27],color = 'red')
...
for index in range(0,len(SDSS_data[:,0])):
    if SDSS_data[index,0] <= .4 and calculated[index,1] <= -22.6:     #red
...
for index,galaxy_index in enumerate(selected_galaxies):
    ...
    z, zErr, petroMag_r, petroMagErr_r, cModelMag_u, cModelMagErr_u, petroRad_r, petroRadErr_r, modelMag_r, modelMag_u = SDSS_data[galaxy_index,:]
